...
# Description: tcp bbr enhancement CentOS 6 / 7 branch
# Version: 1.1.1
# Author: nanqinlang
...
check_system(){
	...
	#number
	[[ ! -z "`cat /etc/redhat-release | grep -E -i " 7."`" ]] && bit=7
	[[ ! -z "`cat /etc/redhat-release | grep -E -i " 6."`" ]] && bit=6
	#bit
	[[ "`uname -m`" != "x86_64" ]] && echo -e "${Error} only support CentOS 64bit!" && exit 1
}
...
check_kernel(){
	...
	[[ "${bit}" = "6" ]] && sed -i "s/^default.*/default=0/" /boot/grub/grub.conf
}
...
install_image(){
	[[ ! -f kernel-ml-4.13.8-1.el${bit}.elrepo.x86_64.rpm ]] && wget http://elrepo.org/linux/kernel/el${bit}/x86_64/RPMS/kernel-ml-4.13.8-1.el${bit}.elrepo.x86_64.rpm
	yum  install -y kernel-ml-4.13.8-1.el${bit}.elrepo.x86_64.rpm
}
install_devel(){
	[[ ! -f kernel-ml-devel-4.13.8-1.el${bit}.elrepo.x86_64.rpm ]] && wget http://elrepo.org/linux/kernel/el${bit}/x86_64/RPMS/kernel-ml-devel-4.13.8-1.el${bit}.elrepo.x86_64.rpm
	yum  install -y kernel-ml-devel-4.13.8-1.el${bit}.elrepo.x86_64.rpm
}
install_headers(){
	[[ ! -f kernel-ml-headers-4.13.8-1.el${bit}.elrepo.x86_64.rpm ]] && wget http://elrepo.org/linux/kernel/el${bit}/x86_64/RPMS/kernel-ml-headers-4.13.8-1.el${bit}.elrepo.x86_64.rpm
	yum  install -y kernel-ml-headers-4.13.8-1.el${bit}.elrepo.x86_64.rpm
}
