...
check_ovz(){
	...
	virt=`virt-what`
	if [[ "${virt}" = "openvz" ]]; then
	echo -e "${Error} OpenVZ not support!" && exit 1
...
get_version(){
	echo -e "${Info} input kernel version you want (defaultly use 4.10.2):"
...
delete_surplus_image(){
	...
	else echo -e "${Error} check image failed !" && exit 1
	fi
}
...
check_kernel(){
	...
	else echo -e "${Info} required is v4.10.2, noneed headers"
	fi
...
ver_4.10.2(){
	[[ ! -e /lib/modules/`uname -r`/kernel/net/ipv4/tcp_nanqinlang.ko ]] && echo -e "${Info} loading mod" && cd /lib/modules/`uname -r`/kernel/net/ipv4 && wget -O tcp_nanqinlang.ko "https://raw.githubusercontent.com/nanqinlang-tcp/tcp_nanqinlang/master/Debian/mod/tcp_nanqinlang_for_4.10.2.ko" && insmod tcp_nanqinlang.ko && depmod -a
...
compiler(){
	...
	if [[ -z "${ver_4_13}" ]]; then
		wget https://raw.githubusercontent.com/nanqinlang-tcp/tcp_nanqinlang/master/Debian/general/tcp_nanqinlang-gentle.c
	else
		wget https://raw.githubusercontent.com/nanqinlang-tcp/tcp_nanqinlang/master/Debian/kernel-v4.13/tcp_nanqinlang-gentle.c
	fi
